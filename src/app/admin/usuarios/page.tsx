...
export default async function AdminUsuariosPage() {
  ...
  return (
    <div className="p-8 max-w-6xl">
      ...
      <div className="border border-[#4d4635]">
        ...
        <div className="grid grid-cols-[1fr_1fr_80px_70px_80px_100px] gap-4 px-4 py-3 border-b border-[#4d4635] bg-[#1c1b1b]">
          {['Usuario', 'Email', 'Rol', 'Momentos', 'Pujas', 'Valor'].map((h) => (
            <LabelCaps key={h} className="text-[#4d4635] text-[9px]">{h}</LabelCaps>
          ))}
        </div>
        ...
        {users.map((user) => {
          ...
          return (
            ...
            >
              ...
              <span className={`label-caps text-[9px] ${ROLE_COLOR[user.role] ?? 'text-[#4d4635]'}`}>
                {user.role}
              </span>
      ...
      {/* Link to curator profiles */}
      <p className="mt-6 text-xs text-[#4d4635]">
        Haz clic en un usuario para ver su perfil público:{' '}
        <Link href="/curator" className="text-[#99907c] hover:text-[#f2ca50] transition-colors">
          /curator/[id]
        </Link>
      </p>
    </div>
